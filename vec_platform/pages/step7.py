...
def step7_layout(session_id: str | None):
    ...
    return html.Div([
        ...
        # with Phase 3.9's q6_fairness_pref and didn't have a clean
        # rationale for living here on Step 7. q6_fairness_pref on Step 8
        # is now the canonical fairness measure.)
        html.Hr(),
        dbc.Card([
            dbc.CardBody([
                html.H4(
...
def submit_step7(n_clicks, q, search):
    """Upsert Q7 (broader-impacts shift) onto survey_responses, then nav
    to /dash/step8."""
    if not n_clicks:
